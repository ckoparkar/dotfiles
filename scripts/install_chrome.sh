...
# 1 RPM package from Fedora 17 if not already fully downloaded.

# It then copies 11 libraries from the F15/F17 packages into /opt/google/chrome/lib.
...
# It sets SELinux context user/type for the F15/F17 libraries as well if SELinux
...
#   gnome_keyring_attribute_list_new function, which didn't exist until Fedora 17's
#   libgnome-keyring.so.0 library! Luckily, the F17 library works in CentOS 6,
#   so that's been added and the chrome-deps RPM has been bumped to version 1.20.
...
# $1 = Original $0 (i.e. script name)
{
   ...
   fedver=15 # Fedora version with most of the needed libraries (F16 doesn't work!)
...
# Display final installation messages before exiting
{
   ...
   echo "$chrome_name and Fedora $fedver/$keyringver libraries installed successfully."
